...
/*
 *
 * E0.21	RGBA LED PWM color mixer CTMU touch driver
 * For MPLABX 1.41
 * ***		background I/O using timer0 and adc interrupts
...
#define	TIMERCHARGE_BASE_4		62543		// .55 uA time, small plate low sens ~700us
#define	TIMERDISCHARGE			45534		// discharge and max touch data update period 10ms

#define TRIP 32  //Difference between pressed
//and un-pressed switch
#define HYST 31 //amount to change
//from pressed to un-pressed
#define PRESSED 1
#define UNPRESSED 0
#define	CHOP_BITS	3						// remove this many bits to reduce noise from the touch sensor
...
unsigned char vspeed = RNG_SPEED_F, MAXA = 255, MAXR = 255, MAXG = 255, MAXB = 255, ctmu_button;
volatile unsigned char CTMU_ADC_UPDATED = FALSE, TIME_CHARGE = FALSE, CTMU_WORKING = FALSE;
volatile unsigned int touch_base[16], switchState, charge_time[16]; //storage for reading parameters
...
void high_handler(void)
{
	...
	timer.lt = ADRES; // Get the value from the A/D
	timer.lt = (timer.lt >> CHOP_BITS)&0x00ff; // toss lower bit noise and mask
	if (timer.bt[0] < (touch_base[channel] - TRIP)) { // see if we have a pressed button
	    switchState = PRESSED;
	    LATCbits.LATC4 = OFF;
	} else if (timer.bt[0] > (touch_base[channel] - TRIP + HYST)) {
	    switchState = UNPRESSED;
	    LATCbits.LATC4 = ON;
	}
	TMR3H = timer.bt[1] | ((channel << 4)&0xf0); // copy high byte/channel data [4..7] bits
...
int touch_base_calc(unsigned char channel)
{
    static union Timers timer;
...
int ctmu_touch(unsigned char channel)
{
    unsigned int ctmu_change;
    static union Timers timer;

    timer.bt[0] = TMR3L; // read low byte and read 16bits from timer counter into TMR3 16bit buffer
    timer.bt[1] = TMR3H; // read high byte

    ctmu_change = touch_base[channel] - timer.bt[0]; // read diff
    if (ctmu_change < 0) ctmu_change = 0;
    LATD = ~ctmu_change;
    return ctmu_change;
}
...
void main(void)
{
    ...
    ctmu_setup(11, ctmu_button); // config the CTMU for touch response 1 for normal .55ua, 2 for higher 5.5ua charge current
	...
	touch_tmp = ctmu_touch(ctmu_button);
	LATCbits.LATC6 = !LATCbits.LATC6; // blink led
	if (touch_tmp > 1) {
